...
class BaseProxy {
   ...
   * @param {Object} options axios setting for this resource.
   */
   ...
   */
  submit(requestType, url, data = null, options = this.options) {
    return new Promise((resolve, reject) => {
      $http[requestType](url + this.getParameterString(), data, options)
        .then((response) => {
  ...
  getParameterString() {
    ...
    // .filter(key => !!this.parameters[key]) // method removing empty or zero param key
